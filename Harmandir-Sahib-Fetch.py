...
def fetch_latest_api_data():
    ...
    try:
        ...
        # Gather ALL recent videos from the fetched batch that match EITHER target title
        candidates = []
        ...
        # Loop through ALL candidates and collect valid ones
        for candidate in candidates:
            ...
            if is_live or get_total_seconds(duration_iso) >= MIN_DURATION_SECONDS:
                ...
                payload = {
                    ...
                    "timeAgo": str(published_time_ms),                   # String (Publish time)
        ...
        # 🎯 SORTING LOGIC: Find the best candidate
        # Priority 1: is_live (True > False)
        # Priority 2: published_time_ms (Higher/Newer > Lower/Older)
        # Priority 3: is_primary (Primary > Secondary)
        valid_candidates.sort(
            key=lambda x: (x["is_live"], x["published_time_ms"], x["is_primary"]), 
            reverse=True
        )

        # Return the payload of the absolute best candidate
