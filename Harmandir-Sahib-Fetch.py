...
def build_video_payload(rss_videos, yt_videos):
    ...
    latest_time = None
    live_published_dt = None

    # Perfected live logic incorporated here
    for v in rss_videos:
        ...
        if live_status == "live":
            live_candidate = yt
            live_published_dt = v["published"]
            break

    final_yt = live_candidate if live_candidate else latest_candidate
    published_dt = live_published_dt if live_candidate else latest_time
...
def update_firestore_dual(payload):
    ...
    def do_update(db_client, collection_name, app_name, data):
        docs = (
            ...
            .where(filter=FieldFilter("gurbani_live", "==", CHANNEL_ID)) # Assuming channel_Id is the linking key
            .limit(1)
            .get()
        )
