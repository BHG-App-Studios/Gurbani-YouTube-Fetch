...
def fetch_latest_5_matching():
    ...
    for entry in root.findall("atom:entry", NS):
        ...
        # ✅ FILTER: Takht Sri Patna Sahib ONLY (UNCHANGED)
        if "Takht Sri Patna Sahib" not in title:
            continue
...
def update_firestore(data):
    ...
    if existing.get("url") == data["url"]:
        print("⏭ No change detected (same Takht Sri Patna Sahib). Skipping update.")
        return
    ...
    print("✅ Takht Sri Patna Sahib updated successfully")

# ---------------- MAIN ----------------
if __name__ == "__main__":

    print("🔄 Fetching latest Takht Sri Patna Sahib videos from RSS...")
    rss_videos = fetch_latest_5_matching()

    if not rss_videos:
        print("❌ No Takht Sri Patna Sahib video found")
        exit(0)
    ...
    print("🎯 Selected Takht Sri Patna Sahib:")
    print(final_video)
