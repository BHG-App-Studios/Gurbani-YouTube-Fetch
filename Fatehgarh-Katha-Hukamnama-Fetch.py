...
# ---------------- CONFIG ----------------
CHANNEL_ID = "UCxddWE7n0JFZo6yadIPFdfQ"
RSS_URL = f"https://www.youtube.com/feeds/videos.xml?channel_id={CHANNEL_ID}"

MIN_DURATION_SECONDS = 90  # ⏱️ 1.5 minutes (90 seconds)
...
def fetch_latest_hukamnama_katha():
    ...
    # ✅ ORIGINAL LOGIC FILTER: Valid titles to match against
    valid_title_phrases = [
        "Hukamnama vyakhya From Sri Fatehgarh Sahib",
        "Hukamnama from Sri Fatehgarh Sahib Today"
    ]
    ...
    for entry in root.findall("atom:entry", NS):
        ...
        # ✅ Check if ANY of the valid phrases exist in the title
        if not any(phrase in title for phrase in valid_title_phrases):
            continue
    ...
    for match in matches:
        ...
        if duration_sec >= MIN_DURATION_SECONDS:
            ...
            print(f"⏭️ Skipping video (Duration: {duration_sec}s < {MIN_DURATION_SECONDS}s): {vid}")

    if not latest or not latest_yt_data:
        print(f"❌ No matching videos found that meet the {MIN_DURATION_SECONDS}s minimum length requirement.")
        return None
...
def update_firestore_dual(data):
    ...
    def do_update(db_client, collection_name, app_name, payload):
        docs = (
            ...
            # ❗ ORIGINAL LOGIC: Query by 'hukamnama_katha_fatehgarh' field matching channel ID
            .where(filter=FieldFilter("hukamnama_katha_fatehgarh", "==", CHANNEL_ID))
            .limit(1)
            .get()
        )
...
if __name__ == "__main__":
    ...
    if not result:
        print("❌ No Fatehgarh Sahib Hukamnama/Katha video found")
    else:
